xmlport 52012 "SAF-T PT_RESP"
{
    ...
    trigger OnBeforePassVariable()
    begin
        //  InvoiceSystemEntryDate := FormatDate(SalesInvoiceHeader."Posting Date")
        //ELSE
        //  IF SalesInvoiceHeader."Bill-to Customer No." <> '' THEN
        //  ELSE
        IF SalesInvoiceHeader."Creation Date" <> 0D THEN InvoiceSystemEntryDate:=FormatDateTime(SalesInvoiceHeader."Creation Date", SalesInvoiceHeader."Creation Time")
        ELSE
        BEGIN
            IF SalesInvoiceHeader."Source Code" = DeletedDocSourceCode THEN InvoiceSystemEntryDate:=FormatDateTime(SalesInvoiceHeader."Posting Date", 0T)
            ELSE
            BEGIN
                IF SalesInvoiceHeader."Bill-to Customer No." <> '' THEN InvoiceSystemEntryDate:=GetEntryDocDateTime(TRUE, SalesInvoiceHeader."No.", SalesInvoiceHeader."Bill-to Customer No.")
                ELSE
    ...
    trigger OnBeforePassVariable()
    begin
        InvQuantity:=FORMAT("Sales Invoice Line".Quantity, 0, 9);
    end;
    }
    ...
    trigger OnBeforePassVariable()
    begin
        InvCreditAmount:=FORMAT(ROUND("Sales Invoice Line".Amount / DocFactor, 0.01), 0, '<Precision,2:2><Standard Format,9>');
    end;
    }
    textelement(invtax)
    {
    ...
    trigger OnBeforePassVariable()
    begin
        //  CrMemoSystemEntryDate := FormatDate(SalesCrMemoHeader."Posting Date")
        //ELSE
        //  IF SalesCrMemoHeader."Bill-to Customer No." <> '' THEN
        //  ELSE
        IF SalesCrMemoHeader."Creation Date" <> 0D THEN CrMemoSystemEntryDate:=FormatDateTime(SalesCrMemoHeader."Creation Date", SalesCrMemoHeader."Creation Time")
        ELSE
        BEGIN
            IF SalesCrMemoHeader."Source Code" = DeletedDocSourceCode THEN CrMemoSystemEntryDate:=FormatDateTime(SalesCrMemoHeader."Posting Date", 0T)
            ELSE
            BEGIN
                IF SalesCrMemoHeader."Bill-to Customer No." <> '' THEN CrMemoSystemEntryDate:=GetEntryDocDateTime(FALSE, SalesCrMemoHeader."No.", SalesCrMemoHeader."Bill-to Customer No.")
                ELSE
    ...
    trigger OnBeforePassVariable()
    begin
        CrMQuantity:=FORMAT("Sales Cr.Memo Line".Quantity, 0, 9);
    end;
    }
    ...
    trigger OnBeforePassVariable()
    begin
        CrMDebitAmount:=FORMAT(ROUND("Sales Cr.Memo Line".Amount / DocFactor, 0.01), 0, '<Precision,2:2><Standard Format,9>');
    end;
    }
    textelement(crmtax)
    {
 ...
 TotalDebitDocs+=ROUND(SalesCrMemo.Amount / SalesCrMemo."Currency Factor", 0.01)
    ...
    begin
        ...
        IF GLEntry.FindSet()then REPEAT GLReg.Reset();
                ...
                IF GLReg.FindFirst()then BEGIN
                    TempGLReg.SETRANGE("No.", GLReg."No.");
    ...
    begin
        ...
        //  XMLDOMMgmt.AddAttribute(XMLRootNode,'xmlns','urn:OECD:StandardAuditFile-Tax:PT_1.01_01');
