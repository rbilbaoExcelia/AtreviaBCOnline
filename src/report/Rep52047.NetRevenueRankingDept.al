report 52047 "Net Revenue Ranking Dept"
{
    // 141 OS.RM 07/06/2017 FIN.011 Informe Ranking Facturación Neta
    //     OS.OA 19/09/2017 FIN.012 Modificaciones
    DefaultLayout = RDLC;
    RDLCLayout = './src/report/layouts/NetRevenueRankingDept.rdlc';
    Caption = 'Net Revenue Ranking by Office/Dept.t';
    ...
    dataset
    {
        ...
        {
            DataItemTableView = SORTING("Dimension Code", Code)ORDER(Ascending)WHERE("Global Dimension No."=CONST(1), "Dimension Value Type"=CONST(Standard), Blocked=CONST(false));
            ...
            {
                //DataItemLink = '';
                DataItemTableView = SORTING("Dimension Code", Code)ORDER(Ascending)WHERE("Global Dimension No."=CONST(2), "Dimension Value Type"=CONST(Standard), Blocked=CONST(false));
                PrintOnlyIfDetail = true;
                ...
                dataitem("Company"; Company)
                {
                    ...
                    dataitem("G/L Entry Year";17)
                    {
                        DataItemLink = "Global Dimension 1 Code"=FIELD(Code);
                        DataItemLinkReference = "Dimension Value";
                        DataItemTableView = SORTING("G/L Account No.", "Global Dimension 1 Code", "Global Dimension 2 Code", "Posting Date")ORDER(Ascending)WHERE("Job Type"=FILTER(Periodical|"One Off"));

                        trigger OnAfterGetRecord()
                        begin
                            IF NOT job.GET("G/L Entry Year"."Job No.")THEN BEGIN
                                CLEAR(job);
                                CurrReport.Skip();
                            END;
                            IF NOT recCust.GET(job."Bill-to Customer No.")THEN CLEAR(recCust);
                            IF NOT CustPriceGroup.GET(job."Customer Price Group")THEN CLEAR(CustPriceGroup);
                            //Un año.
                            MemInt.Init();
                            MemInt."Cod 1":="G/L Entry Year"."Global Dimension 1 Code";
                            IF job."Customer Price Group" <> '' THEN MemInt."Cod 2":=job."Customer Price Group"
                            ELSE
                                MemInt."Cod 2":=job."Bill-to Customer No.";
                            IF(breakDown AND ("Dimension Value"."Split Dimension"))THEN MemInt."Cod 3":="G/L Entry Year"."Global Dimension 2 Code"
                            ELSE
                                MemInt."Cod 3":=''; //019>
                            ...
                            MemInt."Value 3"+=-"G/L Entry Year".Amount;
                            MemInt.Modify();
                        end;
                        ...
                        begin
                            ...
                            IF(breakDown AND ("Dimension Value"."Split Dimension"))THEN SETRANGE("Global Dimension 2 Code", "Dimension Value 2".Code); //019>
                            SETFILTER("G/L Account No.", JobSetup."G/L Accounts Rep. Filter AT");
                            SETRANGE("Posting Date", DateFilter1, DateFilter2);
                        end;
                    }
                    dataitem("G/L Entry Past Year";17)
                    {
                        DataItemLink = "Global Dimension 1 Code"=FIELD(Code);
                        DataItemLinkReference = "Dimension Value";
                        DataItemTableView = SORTING("G/L Account No.", "Global Dimension 1 Code", "Global Dimension 2 Code", "Posting Date")ORDER(Ascending)WHERE("Job Type"=FILTER(Periodical|"One Off"));
                        ...
                        trigger OnAfterGetRecord()
                        begin
                            IF NOT job.GET("G/L Entry Past Year"."Job No.")THEN BEGIN
                                CLEAR(job);
                                CurrReport.Skip();
                            END;
                            IF NOT recCust.GET(job."Bill-to Customer No.")THEN CLEAR(recCust);
                            ...
                            PastMemInt."Value 3"+=-"G/L Entry Past Year".Amount;
                            PastMemInt.Modify();
                        end;
                        trigger OnPreDataItem()
                        begin
                            JobSetup.Get();
                            IF NOT MoreYears THEN CurrReport.Break();
                            CHANGECOMPANY(Company.Name);
                            IF(breakDown AND ("Dimension Value"."Split Dimension"))THEN SETRANGE("Global Dimension 2 Code", "Dimension Value 2".Code);
                            SETFILTER("G/L Account No.", JobSetup."G/L Accounts Rep. Filter AT");
                            SETRANGE("Posting Date", PastDateFilter1, PastDateFilter2);
                        end;
                    }
                }
                dataitem("Integer"; Integer)
                {
                    ...
                    column(DimOffice; "Dimension Value".Alias)
                    {
                    }
                    column(Dim2Dept; "Dimension Value 2".Alias)
                    {
                    }
                    ...
                    begin
                        ...
                        ELSE
                            BooleanPast:=FALSE;
                        IF(breakDown AND ("Dimension Value"."Split Dimension"))THEN RankingNo:="Dimension Value 2"."No. of Ranking Entries"
                        ELSE
                ...
                trigger OnAfterGetRecord()
                begin
                    IF NOT(breakDown AND ("Dimension Value"."Split Dimension" = TRUE))THEN BEGIN
                        //para que salga en blanco en report builder
                        "Dimension Value 2".Alias:='';
                    ...
                    IF CreateExcel THEN BEGIN
                        ...
                        IF(breakDown AND ("Dimension Value"."Split Dimension"))THEN EnterCell(2, 2, "Dimension Value".Alias + ' ' + "Dimension Value 2".Alias, TRUE, FALSE, FALSE)
                        ELSE
                            EnterCell(2, 2, "Dimension Value".Alias, TRUE, FALSE, FALSE);
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    {
                        ToolTip = 'Break down into Depart.';
                        ApplicationArea = All;
                        Caption = 'Break down into Depart.';
                    }
                }
            }
        }
    ...
    begin
        ...
        IF Date1 = 0D THEN ERROR(Error003);
        IF Date2 = 0D THEN ERROR(Error003);
    ...
    begin
        IF CreateExcel THEN BEGIN
            ...
            IF MemInt."Value 3" <> 0 THEN EnterCell(Fila, 1, MemInt."Cod 2", FALSE, FALSE, FALSE)
            ELSE
    ...
    begin
        IF CreateExcel THEN BEGIN
            ...
            //<012
            // Año Actual - Total
            EnterCell(300, 2, TextTotal, TRUE, FALSE, FALSE);
            EnterCell(300, 3, FormatNumber(Total, 2), TRUE, FALSE, FALSE);
            // Año Actual - Continuados
            EnterCell(301, 2, ContinuoCaptionLbl, TRUE, FALSE, FALSE);
            EnterCell(301, 3, FormatNumber(PcjeContinuousNum, 0) + '%', TRUE, FALSE, FALSE);
            // Año Actual - Puntuales
            EnterCell(302, 2, OneOffCaptionLbl, TRUE, FALSE, FALSE);
            EnterCell(302, 3, FormatNumber(PcjeOneOffNum, 0) + '%', FALSE, FALSE, FALSE);
            IF MoreYears THEN BEGIN
                // Año Actual - Total
                EnterCell(300, 6, TextTotal, TRUE, FALSE, FALSE);
                EnterCell(300, 7, FormatNumber(PastTotal, 2), TRUE, FALSE, FALSE);
                // Año Actual - Continuados
                EnterCell(301, 6, ContinuoCaptionLbl, TRUE, FALSE, FALSE);
                EnterCell(301, 7, FormatNumber(PcjePastContinuousNum, 0) + '%', TRUE, FALSE, FALSE);
                // Año Actual - Puntuales
                EnterCell(302, 6, OneOffCaptionLbl, TRUE, FALSE, FALSE);
                EnterCell(302, 7, FormatNumber(PcjePastOneOffNum, 0) + '%', FALSE, FALSE, FALSE);
            //012>
            END;
            //Crear excel o sheet.
            IF NOT FileCreated THEN BEGIN
                //FileName := FileManagement.ServerTempFileName('xlsx');
                IF(breakDown AND ("Dimension Value"."Split Dimension"))THEN TempExcelBuffer.CreateNewBook("Dimension Value".Alias + ' ' + "Dimension Value 2".Alias)
                ELSE
                    TempExcelBuffer.CreateNewBook("Dimension Value".Alias);
                FileCreated:=TRUE;
                IF(breakDown AND ("Dimension Value"."Split Dimension"))THEN TempExcelBuffer.WriteSheet("Dimension Value".Alias + ' ' + "Dimension Value 2".Alias, COMPANYNAME, USERID)
                ELSE
                    TempExcelBuffer.WriteSheet("Dimension Value".Alias, COMPANYNAME, USERID);
