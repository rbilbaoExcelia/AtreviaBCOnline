report 52078 "Sales Invoice - Inforpress New"
{
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; 112)
        {
            ...
            begin
                CustomerLan.Get("Sales Invoice Header"."Sell-to Customer No.");
                if CustomerLan."Language Code" <> '' then
                    CurrReport.LANGUAGE := cLanguage.GetLanguageID(CustomerLan."Language Code")
                else
                ...
                IF Bank.IBAN <> '' THEN
                    TxtBank[1] := STRSUBSTNO(Text50003, Bank."SWIFT Code", GetBankno)
                ELSE
                    TxtBank[1] := '';
                IF (PaymentMethod."Show Address") AND (Bank.Address <> '') THEN
                    TxtBank[2] := STRSUBSTNO(Text50000, Bank.Address + ' ' + Bank."Address 2" + ' ' + Bank.City)
                ELSE
                    TxtBank[2] := '';
                ...
                IF Job.GET("Sales Invoice Header"."Job No.") THEN
                    CASE Job."Job Type AT" OF
                        Job."Job Type AT"::Seminar:
                            TXTSeminario := SalesSetup."Seminar Foot Text";
                        Job."Job Type AT"::Salon:
                            TXTSeminario := SalesSetup."Salon Foot Text";
                        ELSE
                            TXTSeminario := '';
                    END;
                CLEAR(mTxt);
                ...
                IF SalesLine.FindFirst() then
                    REPEAT
                        IF SalesLine."Job Assistant" <> '' THEN isAssistant := TRUE;
    ...
    local procedure FormatNumber(Q: Decimal; Decim: Integer) T: Text
    var
