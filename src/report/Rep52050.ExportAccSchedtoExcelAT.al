report 52050 "Export Acc. Sched. to Excel AT"
{
    ...
    dataset
    {
        dataitem("Integer"; "Integer")
        {
            ...
            begin
                ...
                GLSetup.Get();
                RowNo:=1;
                //EnterCell(RowNo, 1, Text001, false, false, true, false, '', TempExcelBuffer."Cell Type"::Text); //EX-AMT 22122022
                ...
                if((AccSchedName."Analysis View Name" = '') and (GLSetup."Global Dimension 1 Code" <> '')) or ((AccSchedName."Analysis View Name" <> '') and (AnalysisView."Dimension 1 Code" <> ''))then EnterFilterInCell(RowNo, AccSchedLine.GetFilter("Dimension 1 Filter"), GetDimFilterCaption(1), '', TempExcelBuffer."Cell Type"::Text);
                if((AccSchedName."Analysis View Name" = '') and (GLSetup."Global Dimension 2 Code" <> '')) or ((AccSchedName."Analysis View Name" <> '') and (AnalysisView."Dimension 2 Code" <> ''))then EnterFilterInCell(RowNo, AccSchedLine.GetFilter("Dimension 2 Filter"), GetDimFilterCaption(2), '', TempExcelBuffer."Cell Type"::Text);
                if(AccSchedName."Analysis View Name" = '') or ((AccSchedName."Analysis View Name" <> '') and (AnalysisView."Dimension 3 Code" <> ''))then EnterFilterInCell(RowNo, AccSchedLine.GetFilter("Dimension 3 Filter"), GetDimFilterCaption(3), '', TempExcelBuffer."Cell Type"::Text);
                if(AccSchedName."Analysis View Name" = '') or ((AccSchedName."Analysis View Name" <> '') and (AnalysisView."Dimension 4 Code" <> ''))then EnterFilterInCell(RowNo, AccSchedLine.GetFilter("Dimension 4 Filter"), GetDimFilterCaption(4), '', TempExcelBuffer."Cell Type"::Text);
                RowNo:=RowNo + 1;
                if UseAmtsInAddCurr then EnterFilterInCell(RowNo, GLSetup."Additional Reporting Currency", Currency.TableCaption(), '', TempExcelBuffer."Cell Type"::Text)
                else
                    EnterFilterInCell(RowNo, GLSetup."LCY Code", Currency.TableCaption(), '', TempExcelBuffer."Cell Type"::Text);
                //RowNo := RowNo + 1; EX-AMT 22122022
                RowNo:=6;
                if AccSchedLine.Find('-')then begin
                    ...
                    repeat RecNo:=RecNo + 1;
                        ...
                        if ColumnLayout.Find('-')then repeat CalcColumnValue();
                                ColumnNo:=ColumnNo + 1;
                                EnterCell(RowNo, ColumnNo, MatrixMgt.FormatAmount(ColumnValue, ColumnLayout."Rounding Factor", UseAmtsInAddCurr), AccSchedLine.Bold, AccSchedLine.Italic, AccSchedLine.Underline, AccSchedLine."Double Underline", /*''*/ '#,##0.00', TempExcelBuffer."Cell Type"::Number)until ColumnLayout.Next() = 0;
                    until AccSchedLine.Next() = 0;
                end;
                Window.Close();
                if DoUpdateExistingWorksheet then begin
                    //TempExcelBuffer.UpdateBookExcel(ServerFileName, SheetName, false);
                    //TempExcelBuffer.OpenBookStream(InStreamExcel, SheetName);
                    //TempExcelBuffer.SetCurrent(0, 0);
                    TempExcelBuffer.CreateNewBookWithoutOpen(SheetName);
                    TempExcelBuffer.WriteSheet('', CompanyName, UserId);
                    TempExcelBuffer.CloseBook();
                    if not TestMode then //TempExcelBuffer.OpenExcelWithName(ClientFileName);
 TempExcelBuffer.OpenExcel();
                ...
                begin
                    ...
                    TempExcelBuffer.CloseBook();
                    if not TestMode then TempExcelBuffer.OpenExcel();
                end;
            end;
        }
    }
    ...
    begin
        ...
        if AccSchedLine.Totaling = '' then ColumnValue:=0
        else
        begin
    ...
    begin
        if Filter <> '' then begin
            RowNo:=RowNo + 1;
            // EnterCell(RowNo, 1, FieldName, false, false, false, false, '', TempExcelBuffer."Cell Type"::Text); EX-AMT 22122022 INICIO
            // EnterCell(RowNo, 2, Filter, false, false, false, false, Format, CellType);
            EnterCell(RowNo, 1, Filter, false, false, false, false, Format, CellType); //EX-AMT 22122022 FIN
        end;
    end;
    ...
    local procedure UploadClientFile(var ClientFileName: Text; var ServerFileName: Text): Boolean var
        FromFile: Text[100];
    begin
        UploadIntoStream(Text002, '', '', FromFile, InStreamExcel);
        if FromFile <> '' then begin
            ClientFileName:=FileMgt.GetFileName(FromFile);
            SheetName:=TempExcelBuffer.SelectSheetsNameStream(InStreamExcel);
